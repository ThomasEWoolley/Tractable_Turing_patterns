...
par=           [ 0.5]; % Parameters used in functions and Jacobian
...
% for i=1:22
%  huclean(p); % Set up figures in nice way
p=swibra(Fname,'bpt1',[Fname,'_bp1'],-0.001);
p.plot.bpcmp=2;
p.plot.pcmp=1; % Component for plotting
...
p=cont(p,100);
%%
p=swibra([Fname,'_bp1'],'bpt1',[Fname,'_bp1_bp1'],-0.001);
p.plot.bpcmp=2;
p.plot.pcmp=1; % Component for plotting
p.nc.dsmax=0.05; % Arclength max steplength
p.nc.lammax=20; % Max parameter value
...
r=[0.06*K/L^2 N; N K/L^2]*[u1;u2]-p.mat.M*f; % putting the equation together
end
