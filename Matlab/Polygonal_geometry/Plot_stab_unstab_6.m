...
load Plotting_indices_n6p3.mat
...
ps.mesh.bp=[real((ps.mesh.bp(1,:)+1i*ps.mesh.bp(2,:)).*exp(1i*2*pi/6));imag((ps.mesh.bp(1,:)+1i*ps.mesh.bp(2,:)).*exp(1i*2*pi/6))];
...
load Plotting_indices_n6p4.mat
[~,I]=sort(Possible_vecs(:,5));
Possible_vecs=Possible_vecs(I,:);
Unstable_sol_index=Possible_vecs(Possible_vecs(:,3)==1,:);
Stable_sol_index=Possible_vecs(Possible_vecs(:,3)==0,:);
...
export_fig('./Pictures/n6_stab_unstab.png','-r300')
