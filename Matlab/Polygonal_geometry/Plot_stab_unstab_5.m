...
load Plotting_indices_n5p3.5.mat
...
[ps.u(end), max(ps.u(1:ps.np)), min(ps.u(1:ps.np)),Stable_sol_index(2,1),Stable_sol_index(2,2)]
ps.mesh.bp=[real((ps.mesh.bp(1,:)+1i*ps.mesh.bp(2,:)).*exp(1i*4*pi/5));imag((ps.mesh.bp(1,:)+1i*ps.mesh.bp(2,:)).*exp(1i*4*pi/5))];
...
load Plotting_indices_n5p4.5.mat
[~,I]=sort(Possible_vecs(:,5));
Possible_vecs=Possible_vecs(I,:);
Unstable_sol_index=Possible_vecs(Possible_vecs(:,3)==1,:);
Stable_sol_index=Possible_vecs(Possible_vecs(:,3)==0,:);
...
pdeplot(ps.mesh.bp, ps.mesh.be, ps.mesh.bt,'xydata',ps.u(1:ps.np))
title('$R=4.5$, Stable','interpreter','Latex')
formatter
...
[pu.u(end), max(pu.u(1:pu.np)), min(pu.u(1:pu.np)),Unstable_sol_index(1,1),Unstable_sol_index(1,2)]
pdeplot(pu.mesh.bp, pu.mesh.be, pu.mesh.bt,'xydata',pu.u(1:pu.np))
title('$R=4.5$, Unstable','interpreter','Latex')
formatter

export_fig('./Pictures/n5_stab_unstab.png','-r300')
