...
alpha = 0.02;
n=100;
[theta_u,theta_v,F]=meshgrid(linspace(0,pi,n),linspace(0,pi,n),linspace(G,10,100));

f1=(6.*r_v+8.*cos(theta_v)).*cos(theta_u)-6.*cos(theta_v).*r_u;
f1lin=(-3.*r_v-4).*theta_u.^2+(-4+3.*r_u).*theta_v.^2-6.*r_u+6.*r_v+8;
f2=sqrt(r_u).*sqrt(r_v).*sqrt((3.*r_v+4.*cos(theta_v)).*cos(theta_u)-3.*cos(theta_v).*r_u).*sqrt(F-G)+r_u.*F.*cos(theta_u)+r_v.*cos(theta_v)-(3/4.*(F-G)).*r_u.^2;
f2lin=-3/4.*((((F-G).*r_u.^2+(2/3.*F).*(theta_u.^2-2).*r_u+(2/3.*r_v).*(theta_v.^2-2)).*sqrt(4-3.*r_u+3.*r_v)+sqrt(F-G).*((-theta_v.^2+4).*sqrt(r_v).*r_u.^(3/2)+((theta_u.^2-4).*r_v.^(3/2)+(4/3.*sqrt(r_v)).*(theta_u.^2+theta_v.^2-4)).*sqrt(r_u)))/sqrt(4-3.*r_u+3.*r_v));
