...
f2=sqrt((3.*r_v+4.*cos(theta_v)).*cos(theta_u)-3.*r_u.*cos(theta_v)).*sqrt(r_u).*sqrt(r_v).*sqrt(G-F)-r_u.*F.*cos(theta_u)+r_v.*cos(theta_v)+(1/4.*(3.*F+3.*G)).*r_u.^2+3/2.*(r_v.^2);
f2lin=-3/4.*((((-F-G).*r_u.^2-(2/3.*F).*(theta_u.^2-2).*r_u-(2.*(-(1/3).*theta_v.^2+r_v+2/3)).*r_v).*sqrt(3.*r_v+4-3.*r_u)+sqrt(G-F).*((-theta_v.^2+4).*sqrt(r_v).*r_u.^(3/2)+sqrt(r_u).*((theta_u.^2-4).*r_v.^(3/2)+(4/3.*sqrt(r_v)).*(theta_u.^2+theta_v.^2-4))))/sqrt(3.*r_v+4-3.*r_u));
